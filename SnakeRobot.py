...
class SnakeRobot:
    ...
    def get_imu_data(self, dt, add_noise=False, debug=False):
        ...
        lin_acc = (self.prev_lin_vel[0,:] - lin_vel_world) / dt
        self.prev_lin_vel[0, :] = lin_vel

        lin_acc_world += self.g # Add Gravity Vector
        ...
        if add_noise:
            ...
            lin_acc_world = lin_acc_world + lin_acc_noise
            ang_vel_world = ang_vel_world + ang_vel_noise
        
        # Convert lin acc and ang vel world to link
        ...
        for link_idx in range(num_child_links):
            lin_vel_world, ang_vel_world = p.getLinkState(self._snakeID, 1+2*link_idx, computeLinkVelocity=True)[6:8]

            lin_acc = (self.prev_lin_vel[link_idx + 1,:] - lin_vel_world) / dt
